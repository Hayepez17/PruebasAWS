// Project: iot_ingest
use common_lib::{DatabaseSettings, LoggerDeviceLocation};
...
pub async fn fetch_logger_devices(
    pool: &MySqlPool,
    device_locations: HashSet<u32>,
) -> Result<Vec<LoggerDeviceLocation>, sqlx::Error> {
    let device_locations: Vec<u32> = device_locations.into_iter().collect();

    let placeholders = device_locations
        .iter()
        .map(|_| "?")
        .collect::<Vec<_>>()
        .join(",");

    let query_loggers = format!(
        r#"SELECT * FROM 
        (SELECT * FROM logger WHERE device_location_id IN ({}) ORDER BY last_update DESC LIMIT {}) AS subquery
        GROUP BY device_location_id
        ORDER BY last_update DESC;"#,
        placeholders,
        device_locations.len() * 10
    );

    let mut select_query = sqlx::query_as::<_, LoggerDeviceLocation>(&query_loggers);

    // Vincular las direcciones MAC
    for device_location in device_locations.iter() {
        select_query = select_query.bind(device_location);
    }

    // Ejecutar la consulta y devolver los resultados
    let logger_devices = select_query.fetch_all(pool).await?;
