...
```bash
ingestIotRustSAM$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ingestIotRustSAM$ sam local start-api
ingestIotRustSAM$ curl http://localhost:3000/
```
...
```bash
ingestIotRustSAM$ sam logs -n HelloWorldFunction --stack-name ingestIotRustSAM --tail
```
